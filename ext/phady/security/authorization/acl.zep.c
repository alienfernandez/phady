...
/**
  * @class Phady\Security\Authorization\Acl -  Authorization handlers for app
  *
...
ZEPHIR_INIT_CLASS(Phady_Security_Authorization_Acl) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Phady\\Security\\Authorization, Acl, phady, security_authorization_acl, phalcon_mvc_user_component_ce, phady_security_authorization_acl_method_entry, 0);
	 ...
	 */
	zend_declare_property_null(phady_security_authorization_acl_ce, SL("acl"), ZEND_ACC_PRIVATE TSRMLS_CC);
	 ...
	 * @var string
	 */
	zend_declare_property_null(phady_security_authorization_acl_ce, SL("moduleName"), ZEND_ACC_PRIVATE TSRMLS_CC);
	 ...
	 * @var array
	 */
	zend_declare_property_null(phady_security_authorization_acl_ce, SL("resources"), ZEND_ACC_PRIVATE TSRMLS_CC);

	/**
	 * Accessible to everyone
	 * @var array
	 */
	zend_declare_property_null(phady_security_authorization_acl_ce, SL("publicResources"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @const string
	 */
	zend_declare_class_constant_string(phady_security_authorization_acl_ce, SL("PREFIX_ACL_APP"), "app-acl-" TSRMLS_CC);
...
PHP_METHOD(Phady_Security_Authorization_Acl, __construct) {
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "loadresources", NULL, 44);
...
PHP_METHOD(Phady_Security_Authorization_Acl, isAllowed) {
	...
	zephir_is_iterable(roles, &_1, &_0, 0, 0, "phady/security/authorization/acl.zep", 112);
	for (
