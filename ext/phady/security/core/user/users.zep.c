...
PHP_METHOD(Phady_Security_Core_User_Users, getUsers) {
	...
	ZEPHIR_CALL_METHOD(&arrUsers, userRepository, "getusers", NULL, 87, arrDataFilter);
	zephir_check_call_status();
	...
	if (getTotal) {
		ZEPHIR_CALL_METHOD(&arrTotalUsers, userRepository, "gettotalusers", NULL, 88, arrData);
		zephir_check_call_status();
