...
PHP_METHOD(Phady_Console_Command, background) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "php_uname", NULL, 17);
	...
	if (ZEPHIR_IS_STRING(_3, "Windows")) {
		...
		ZEPHIR_SINIT_VAR(_5);
		ZVAL_STRING(&_5, "r", 0);
		ZEPHIR_CALL_FUNCTION(&_6, "popen", NULL, 18, _4, &_5);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(NULL, "pclose", NULL, 19, _6);
		zephir_check_call_status();
	} else {
		if (priority) {
			...
			ZEPHIR_CALL_FUNCTION(&pID, "shell_exec", &_7, 20, _4);
			zephir_check_call_status();
		} else {
			ZEPHIR_INIT_VAR(_8);
			ZEPHIR_CONCAT_SVS(_8, "nohup ", command, " > /dev/null & echo $!");
			ZEPHIR_CALL_FUNCTION(&pID, "shell_exec", &_7, 20, _8);
			zephir_check_call_status();
		}
	}
	RETURN_CCTOR(pID);
...
PHP_METHOD(Phady_Console_Command, isRunning) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "exec", NULL, 21, _0, processState);
...
PHP_METHOD(Phady_Console_Command, kill) {
	...
	if (zephir_is_true(_0)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "exec", NULL, 21, _1);
		zephir_check_call_status();
		RETURN_MM_BOOL(1);
	}
	RETURN_MM_BOOL(0);
