	...
	if (_0) {
		...
		if (first == 0) {
			if (Z_TYPE_P(posResultQuery) != IS_NULL) {
				...
				ZEPHIR_CALL_FUNCTION(&strQuery, "preg_replace", &_9, 130, _7, _8, text);
			...
			if (ZEPHIR_IS_LONG(posResultQuery, 0)) {
				...
				ZEPHIR_INIT_VAR(_11);
				ZVAL_LONG(_11, 1);
				ZEPHIR_CALL_FUNCTION(&strQuery, "preg_replace", &_9, 130, _7, _8, text, _11);
				zephir_check_call_status();
			} else {
...
PHP_METHOD(Phady_Util_Text, findAndReplaceUrlTags) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "preg_replace", &_3, 130, _0, _1, sanitate);
...
PHP_METHOD(Phady_Util_Text, snakeToCamel) {
	...
	if (_1 <= _2) {
		while (1) {
			...
			ZEPHIR_CALL_FUNCTION(&_6, "mb_strtoupper", &_7, 135, _5);
			zephir_check_call_status();
			ZEPHIR_SINIT_NVAR(_8);
			ZVAL_LONG(&_8, 1);
			ZEPHIR_CALL_FUNCTION(&_9, "mb_substr", &_10, 136, p, &_8);
			zephir_check_call_status();
...
PHP_METHOD(Phady_Util_Text, camelToSnake) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "preg_replace", NULL, 130, _0, _1, camel);
	zephir_check_temp_parameter(_0);
	zephir_check_temp_parameter(_1);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 137, _2);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Text, toUpper) {
	...
	if (Z_TYPE_P(encoding) == IS_STRING) {
		...
		ZEPHIR_CALL_FUNCTION(&enc, "mb_internal_encoding", NULL, 132);
		zephir_check_call_status();
	}
	ZEPHIR_RETURN_CALL_FUNCTION("mb_strtoupper", NULL, 135, text, enc);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Text, toLower) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 137, text, enc);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Text, upperCaseFirst) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_LONG(&_0, 2);
	ZEPHIR_CALL_FUNCTION(&newText, "mb_convert_case", &_1, 138, text, &_0, enc);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_3, &_2)
		...
		ZVAL_LONG(&_0, 2);
		ZEPHIR_CALL_FUNCTION(&_6, "mb_convert_case", &_1, 138, e, &_0, enc);
		zephir_check_call_status();
		ZEPHIR_SINIT_NVAR(_0);
		ZVAL_LONG(&_0, 1);
		ZEPHIR_CALL_FUNCTION(&_7, "mb_convert_case", &_1, 138, e, &_0, enc);
		zephir_check_call_status();
...
PHP_METHOD(Phady_Util_Text, countInstances) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("mb_substr_count", NULL, 139, haystack, needle);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Text, stripos) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "mb_strtoupper", &_1, 135, haystack);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_2, "mb_strtoupper", &_1, 135, needle);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_3);
	ZVAL_LONG(&_3, offset);
	ZEPHIR_RETURN_CALL_FUNCTION("mb_strpos", NULL, 140, _0, _2, &_3);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Text, left) {
	...
	ZEPHIR_CALL_FUNCTION(&len, "mb_strlen", NULL, 141, text);
	zephir_check_call_status();
	if (ZEPHIR_GT_LONG(len, left)) {
		...
		ZEPHIR_SINIT_VAR(_1);
		ZVAL_LONG(&_1, left);
		ZEPHIR_CALL_FUNCTION(&_2, "mb_substr", NULL, 136, text, &_0, &_1);
		zephir_check_call_status();
...
PHP_METHOD(Phady_Util_Text, hash_equals) {
	...
	zval *knownString, *userString, *knownLength = NULL, *userLength = NULL, *i = NULL, compare = zval_used_for_init, *_2 = NULL, *_3, *_4 = NULL, *_6, *_7 = NULL;

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &knownString, &userString);



	if ((zephir_function_exists_ex(SS("hash_equals") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("hash_equals", NULL, 0, knownString, userString);
		zephir_check_call_status();
		RETURN_MM();
	} else {
		if (zephir_fast_strlen_ev(knownString) != zephir_fast_strlen_ev(userString)) {
			RETURN_MM_BOOL(0);
		} else {
			ZEPHIR_CALL_SELF(&knownLength, "safestrlen", NULL, 0, knownString);
			zephir_check_call_status();
			ZEPHIR_CALL_SELF(&userLength, "safestrlen", NULL, 0, userString);
			zephir_check_call_status();
			if (!ZEPHIR_IS_IDENTICAL(userLength, knownLength)) {
				RETURN_MM_BOOL(0);
			}
			ZEPHIR_SINIT_VAR(compare);
			ZVAL_LONG(&compare, 0);
			ZEPHIR_CPY_WRT(_2, knownLength);
			_1 = zephir_get_numberval(_2);
			_0 = 0;
			if (_1 >= 0) {
				while (1) {
					if (_0) {
					...
					ZEPHIR_INIT_NVAR(i);
					ZVAL_LONG(i, _1);
					if (ZEPHIR_IS_LONG_IDENTICAL(&compare, 0)) {
						zephir_array_fetch(&_3, knownString, i, PH_NOISY | PH_READONLY, "phady/util/text.zep", 345 TSRMLS_CC);
						ZEPHIR_CALL_FUNCTION(&_4, "ord", &_5, 142, _3);
						zephir_check_call_status();
						zephir_array_fetch(&_6, userString, i, PH_NOISY | PH_READONLY, "phady/util/text.zep", 345 TSRMLS_CC);
						ZEPHIR_CALL_FUNCTION(&_7, "ord", &_5, 142, _6);
						zephir_check_call_status();
						ZEPHIR_SINIT_NVAR(compare);
						zephir_bitwise_xor_function(&compare, _4, _7 TSRMLS_CC);
					}
				}
			}
			RETURN_MM_BOOL(ZEPHIR_IS_LONG_IDENTICAL(&compare, 0));
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phady_Util_Text, safeStrlen) {
	...
	if (zephir_is_true(funcExists)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("mb_strlen", NULL, 141, text, &_0);
		zephir_check_call_status();
		RETURN_MM();
	}
