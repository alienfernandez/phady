...
PHP_METHOD(Phady_Util_Out, htmlentities) {
	...
	if (preserve_encoded_entities) {
		...
		if (zephir_is_true(_1)) {
			...
			ZEPHIR_SINIT_VAR(_2);
			ZVAL_LONG(&_2, 3);
			ZEPHIR_CALL_FUNCTION(&translation_table, "get_html_translation_table", &_3, 109, &_0, &_2);
			zephir_check_call_status();
		} else {
			...
			ZVAL_LONG(&_2, 3);
			ZEPHIR_CALL_FUNCTION(&translation_table, "get_html_translation_table", &_3, 109, &_0, &_2, _4);
			zephir_check_call_status();
		}
		ZEPHIR_SINIT_NVAR(_0);
		ZVAL_LONG(&_0, 38);
		ZEPHIR_CALL_FUNCTION(&_4, "chr", NULL, 110, &_0);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_5);
		ZVAL_LONG(_5, '&');
		zephir_array_update_zval(&translation_table, _4, &_5, PH_COPY | PH_SEPARATE);
		ZEPHIR_CALL_FUNCTION(&_6, "strtr", NULL, 111, text, translation_table);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_7);
		ZVAL_STRING(_7, "/&(?![A-Za-z]{0,4}\\w{2,3};|#[0-9]{2,3};)/", ZEPHIR_TEMP_PARAM_COPY);
		ZEPHIR_INIT_VAR(_8);
		ZVAL_STRING(_8, "&amp;", ZEPHIR_TEMP_PARAM_COPY);
		ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", NULL, 112, _7, _8, _6);
	...
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_LONG(&_0, 3);
	ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 113, text, &_0, _1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phady_Util_Out, mbInternalEncoding) {
	...
	if ((zephir_function_exists_ex(SS("mb_internal_encoding") TSRMLS_CC) == SUCCESS)) {
		...
		if (zephir_is_true(encoding)) {
			ZEPHIR_CALL_FUNCTION(&_0, "mb_internal_encoding", &_1, 114, encoding);
			zephir_check_call_status();
		} else {
			ZEPHIR_CALL_FUNCTION(&_0, "mb_internal_encoding", &_1, 114);
			zephir_check_call_status();
		}
		RETURN_CCTOR(_0);
	}
