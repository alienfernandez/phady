...
PHP_METHOD(Phady_Util_Random, getSeed) {
	...
	ZEPHIR_CALL_FUNCTION(&_3, "strrev", NULL, 133, &_2);
...
PHP_METHOD(Phady_Util_Random, getString) {
	...
	if (useSpace) {
		...
		ZEPHIR_CALL_FUNCTION(&_2, "range", &_3, 56, &_0, &_1);
		zephir_check_call_status();
...
PHP_METHOD(Phady_Util_Random, getStringForPassword) {
	...
	ZEPHIR_INIT_VAR(_4);
	ZVAL_LONG(_4, 25);
	ZEPHIR_CALL_FUNCTION(&_5, "min", &_6, 134, _3, _4);
	zephir_check_call_status();
	...
	if (maxLength == 0) {
		...
		ZEPHIR_INIT_NVAR(_4);
		ZVAL_LONG(_4, 25);
		ZEPHIR_CALL_FUNCTION(&_10, "min", &_6, 134, _3, _4);
		zephir_check_call_status();
