...
PHP_METHOD(Phady_Version, get) {
	...
	ZEPHIR_CALL_SELF(&suffix, "_getspecial", &_0, 105, special);
	zephir_check_call_status();
...
PHP_METHOD(Phady_Version, getId) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "sprintf", &_2, 20, &_0, medium);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_STRING(&_0, "%02s", 0);
	ZEPHIR_CALL_FUNCTION(&_3, "sprintf", &_2, 20, &_0, minor);
	zephir_check_call_status();
...
PHP_METHOD(Phady_Version, getPart) {
	...
	do {
		...
		if (part == 3) {
			...
			ZEPHIR_CALL_SELF(&result, "_getspecial", &_0, 105, _1);
			zephir_check_call_status();
			break;
		}
